...
class CK45(RungeKutta):
    ...
    def _step_impl(self):
        ...
        while not order_accepted:
            ...
            y_assess, err_assess, tol = self._comp_sol_err_tol(
                                        h, self.B_assess[0], self.E_assess[0])
            ...
            if E1 < twiddle[0]*quit[0]:
                ...
                y_assess, err_assess, tol = self._comp_sol_err_tol(
                                        h, self.B_assess[1], self.E_assess[1])
                ...
                if E2 < twiddle[1]*quit[1]:
                    ...
                    if E2 < 1:
                        y_new, err, tol = self._comp_sol_err_tol(
                                    h, self.B_fallback[1], self.E_fallback[1])
                ...
                if E1 < 1:
                    y_new, err, tol = self._comp_sol_err_tol(
                                    h, self.B_fallback[0], self.E_fallback[0])
    ...
    def _compute_error(self, h, E):
        return h*np.dot(self.K[:-1,:].T, E)
    
    
    def _compute_solution(self, h, B):
        return np.dot(self.K[:-1,:].T, B) * h + self.y
    
    
    def _comp_sol_err_tol(self, h, B, E):
        sol = self._compute_solution(h, B)
        err = self._compute_error(h, E)
